  ...
  try {
    const result = await pool.query(`
      SELECT u.id,u.member_id,u.source_type,u.name,u.email,u.phone,u.referral_code,u.utr_number,
             ...
             (SELECT COALESCE(SUM(net_amount),0) FROM transactions WHERE user_id=u.id AND income_type IN ('milestone_commission','jackpot_reward','rank_reward') AND status='credited') AS total_milestone_earned,
